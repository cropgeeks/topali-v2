...
public class FindSequenceDialog extends JDialog implements ActionListener
{
	...
	private JPanel getControls()
	{
		...
		highlight = new JCheckBox(
			Text.GuiDiag.getString("FindSequenceDialog.gui02"),
			Prefs.gui_find_highlight);
		highlight.setToolTipText(Text.GuiDiag.getString("FindSequenceDialog.gui05"));
		highlight.setMnemonic(KeyEvent.VK_S);
		matchCase = new JCheckBox(
			Text.GuiDiag.getString("FindSequenceDialog.gui03"),
			Prefs.gui_find_case);
		matchCase.setToolTipText(Text.GuiDiag.getString("FindSequenceDialog.gui06"));
		matchCase.setMnemonic(KeyEvent.VK_M);
		
		JLabel label = new JLabel(
			Text.GuiDiag.getString("FindSequenceDialog.gui04"));
	...
	private void findSequence()
	{
		...
		if (index == -1)
		{
			String msg = Text.format(Text.GuiDiag.getString(
				"FindSequenceDialog.gui09"), seqName.getText());
