...
public class AutoPartitionDialog extends JDialog implements ActionListener
{
	...
	public AutoPartitionDialog(WinMain winMain, AlignmentData data,
			AnalysisResult result)
	{
		...
		if (data.getResults().size() == 0)
		{
			MsgBox
					.msg(
							"Auto-partitioning the alignment is only possible once "
	...
	private JPanel getControls()
	{
		...
		((JSpinner.NumberEditor) discardSpin.getEditor()).getTextField()
				.setToolTipText(
						"Specifies the minimum size a partition must be "
								+ "to be included");
