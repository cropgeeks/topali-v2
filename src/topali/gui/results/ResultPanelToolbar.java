...
public class ResultPanelToolbar extends JToolBar {
    ...
    public ResultPanelToolbar(ResultPanel resPanel, AlignmentData data,
    ...
    public void enableButtons(boolean bInfo, boolean bReselect, boolean bThres,
	    boolean bAddPart, boolean bAutoPart, boolean bToolTips) {
	...
	aReselect = new AbstractAction() {
		...
		if (MsgBox.yesno(msg, 0) == JOptionPane.YES_OPTION)
		    TOPALi.winMain
			    .menuAnlsReselectSequences(result.selectedSeqs);
	    }
	};
	...
	aThres = new AbstractAction() {
	    public void actionPerformed(ActionEvent e) {
		ThresholdDialog diag = new ThresholdDialog(resPanel,
			result.threshold);
	...
	aToolTips = new AbstractAction() {
		...
		TreeToolTipDialog dialog = new TreeToolTipDialog(
			result.useTreeToolTips, result.treeToolTipWindow, data
