...
abstract class GraphResultsPanel extends JPanel implements IThresholdListener
{
	...
	protected void showToolTipDialog()
	{
		...
		TreeToolTipDialog dialog = new TreeToolTipDialog(
			aResult.useTreeToolTips,
			aResult.treeToolTipWindow,
			data.getSequenceSet().getLength()
		);
	...
	protected void saveGraph()
	{
		...
		while (fc.showSaveDialog(MsgBox.frm) == JFileChooser.APPROVE_OPTION)
		{
			...
			{
				MsgBox.msg("There was an unexpected error while saving graph data:\n "
					+ e, MsgBox.ERR);
			}
	...
	protected abstract void saveCSV(File filename)
		throws Exception;
	
	protected abstract void savePNG(File filename)
		throws Exception;
