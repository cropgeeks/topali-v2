...
public class WinMainTipsPanel extends JPanel implements ActionListener
{
	...
	public WinMainTipsPanel()
	{
		...
		tPanel.setStyle(tPanel.OFFICE2003);
	...
	public static void setDisplayedTips(int tipType)
	{
		switch (tipType)
		{
			...
			case TIPS_ALN: setTips(tips_aln, 0); break;
			case TIPS_TRE: setTips(tips_tre, 0); break;
			case TIPS_DSS: setTips(tips_dss, 0); break;
			case TIPS_JOB: setTips(tips_job, 0); break;			
		}
	}
	...
	private static void setTips(TipsText tipsText, int action)
	{
		...
		{
			tipsText.index--;
			if (tipsText.index < 0)	tipsText.index = tipsText.keys.size()-1;
		}
		
		else if (action == 2)
		{
			tipsText.index++;
			if (tipsText.index >= tipsText.keys.size()) tipsText.index = 0;
		}
	...
	class TipsGradientPanel extends GradientPanel
	{
		TipsGradientPanel()
		{
			...
			bBack = getButton(Icons.BACK1, Icons.BACK2,
				Text.Gui.getString("WinMainTipsPanel.gui02"));
			bNext = getButton(Icons.NEXT1, Icons.NEXT2,
				Text.Gui.getString("WinMainTipsPanel.gui03"));
