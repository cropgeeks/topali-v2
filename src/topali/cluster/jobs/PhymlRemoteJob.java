...
public class PhymlRemoteJob extends RemoteJob
{
	...
	public PhymlRemoteJob(PhymlResult result, AlignmentData data)
	{
		super("topali-phyml", result);
	...
	public String ws_submitJob() throws Exception
	{
		...
		call = getCall();
		call.setOperationName(new QName("topali-phyml", "submit"));
		...
		Tracker.log("SUBMITTED", "PHYML", result.jobId, url);
	...
	{
		call = getCall();
		call.setOperationName(new QName("topali-phyml", "getPercentageComplete"));
	...
	{
		call = getCall();
		call.setOperationName(new QName("topali-phyml", "getResult"));

		String resultXML = (String) call.invoke(new Object[]
		{ result.jobId });
		result = (PhymlResult) Castor.unmarshall(resultXML);
