...
public class Jambe2
{
	...
	public void runJambe() throws Exception
	{
		...
		for (int i = 0; i < total; i++)
		{
			...
			// ///
			/*
			 * if (updateGUI) { oEntropy = computeEntropy(oExtractTopos, i); if
			 * (oEntropy == null) return false;
			 * 
			 * dialog.getLocalGraph().setData( oEntropy.getLocalData(), n1 +
			 * stepSize); }
			 */// ///
	...
	{
		return oEntropy.getHistogramDF();
	}

	public float[] getLocalData()
	{
		return oEntropy.getLocalData();
	}

	public float[][] getHistogramArray()
	{
		return oEntropy.getHistogramArray();
	}
