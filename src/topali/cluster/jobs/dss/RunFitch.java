...
*/
	...
	static void saveData(File wrkDir, int run, DistanceMatrix distance, Tree tree)
		throws Exception
	{
	...
	static double readFitchResult(File wrkDir, int run)
		throws Exception
	{
		wrkDir = new File(wrkDir, "fitch" + run);
	...
	void runFitchScripts(File wrkDir, int windowCount)
		throws Exception
	{
		...
		{
			writeUNIXScript(wrkDir, windowCount);
			writeUNIXScript(new File("."), windowCount);
	...
	private void writeUNIXScript(File wrkDir, int windowCount)
		throws Exception
	{
		BufferedWriter out = new BufferedWriter(
			new FileWriter(new File(wrkDir, "fitch_run.sh")));
	...
	private void writeDOSScript(File wrkDir, int windowCount)
		throws Exception
	{
		BufferedWriter out = new BufferedWriter(
			new FileWriter(new File(wrkDir, "fitch_run.bat")));
	...
	private void writeDOSInputs(File wrkDir)
		throws Exception
	{
		BufferedWriter out = new BufferedWriter(
			new FileWriter(new File(wrkDir, "fitch_normal")));
		out.write("u" + CR + "n" + CR + "p" + CR + "0" + CR + "y");
		out.close();
		
		out = new BufferedWriter(
			new FileWriter(new File(wrkDir, "fitch_optimize")));
