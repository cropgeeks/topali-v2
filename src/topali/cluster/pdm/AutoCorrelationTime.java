...
public class AutoCorrelationTime{
  ...
  // --------------------------------------------------------
	public AutoCorrelationTime(File wrkDir, PDMResult result) throws JambeException
	{
		this.wrkDir = wrkDir;
		this.result = result;
		...
		if (Nsample<=0)
			throw new JambeException("AC01: Nsample <= 0 (Bambe settings for "
	...
	public AutoCorrelationTime(String specifiedFileName) throws JambeException
	{
		...
		if (Nsample<=0)
			throw new JambeException("AC02: Nsample <= 0 (Bambe settings for "
				+ "'number of cycles' and 'sample interval' may be invalid).");
  ...
  // ================= PUBLIC METHODS =================
	...
	{
		System.out.println("Total number of configurations read in = "+Nsample+Nburn);
		System.out.println(Nburn+" configurations have been discarded (burn-in).");
		System.out.println(Nsample+" configurations have been used to compute the autocorrelation time (sampling period).");
		System.out.println("Average autocorrelation time = "+this.showAverage());
		System.out.println("Standard deviation = "+this.showSigma());
		System.out.println("The individual values of the autocorrelation time can be found in file results_ACTs.out.");
		this.printOutAll();
		System.out.println("File results_ACT_Neff contains the effective sample size, the average ACT, its standard deviation, and the total sample size (in this order).");
  ...
  public void printOutAverage(){
	...
	public void printOutResults() throws JambeException
	{
		...
		try
		{
			FileOutputStream outFile = new FileOutputStream(outFileName);
			PrintStream outStream= new PrintStream(outFile);   
			outStream.println(NeffExact+"   "+NeffApprox+"   "+Nsample+"   "+actAverage+"   "+actSigma);
			outStream.close();
		}
		catch (IOException e)
		{
...
//			System.out.println("read: " + e);
	...
	i++;  // labels the data
	if (iSave>Nsample){
		throw new JambeException("AC07: " + fileName + " has wrong length. It "
			+ "is " + iSave + " and must be " + Nsample + ".");
	}
	
	
      } catch (IOException ex) {
      	throw new JambeException("AC08: Unable to read from file " + fileName
      		+ ": " + ex);
      } 
    }
  ...
  public float showSigma(){
    ...
    for (i=0; i<nLength; i++){
      actAverage += ((Float) actVector.elementAt(i)).floatValue();
      act2ndMom += (float) Math.pow(((Float) actVector.elementAt(i)).floatValue(),2);
    }
    actAverage /=  nLength;
    act2ndMom /= nLength;
...
/*	
		int nDiscard;
		BambeInfile oBambeInfile= new BambeInfile();
		oBambeInfile.readTopos("infile");
		nDiscard= oBambeInfile.returnBurn()/oBambeInfile.returnInterval();
		return nDiscard;
*/
	}
...
/*
		...
		BambeInfile oBambeInfile= new BambeInfile();
		oBambeInfile.readTopos("infile");
		nSampleSize= oBambeInfile.returnSampleSize();
		return nSampleSize;
*/
	}
}
