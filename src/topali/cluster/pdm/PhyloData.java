...
public class PhyloData{
  ...
  private File wrkDir;
	...
	public PhyloData(File wrkDir, int windowStart, int windowLength) throws JambeException
	{
		this.wrkDir = wrkDir;
  ...
  // ----------------------------------------------------
	...
	public void checkConsistency() throws JambeException
	{
		...
		if (firstNuc < 0 || lastNuc <= firstNuc || nDNA < lastNuc)
		{
			...
			throw new JambeException("PD02: Wrong assignments of firstNuc, "
  ...
  // ---------------------------------------------------
	...
	public void select(int NreplicateWin) throws JambeException
	{
		...
		{
			throw new JambeException("PD04: Unable to open file dna.dat for reading: " + e);
		}
		...
		{
			throw new JambeException("PD05: Unable to open file dna.in for writing: " + e);
		}
		
//		System.out.println("Window of "+(this.lastNuc-this.firstNuc+1)+" base pairs, from "+this.firstNuc+" to "+this.lastNuc+", now in file dna.in");
	}
 ...
 /* public static void main (String args[]){
    ...
    try{
      ...
      inFile= new FileReader("in.windowStartEnd");
        // deprecated: inStream = new DataInputStream(inFile);
      inStream= new BufferedReader(inFile);
      } catch (IOException ex) {
	gui.MsgBox.dirkError("ERROR in PhyloData: Couldn't read from file in.windowStartEnd");
    }
    // Now read in windowStart and windowLength
    try{
      inputLine= inStream.readLine();
    } catch (IOException ex) {
      gui.MsgBox.dirkError("ERROR: Wrong format of input file");
    }
    StringTokenizer oTokenizer= new StringTokenizer(inputLine);   
    if (oTokenizer.hasMoreTokens()){
      windowStart= Integer.parseInt(oTokenizer.nextToken());
    } else {
      gui.MsgBox.dirkError("ERROR in PhyloData when reading in file in.windowStartEnd");
    }
    if (oTokenizer.hasMoreTokens()){
      windowEnd= Integer.parseInt(oTokenizer.nextToken());
    } else {
      gui.MsgBox.dirkError("ERROR in PhyloData when reading in file in.windowStartEnd");
    }
    windowLength=windowEnd-windowStart+1;
    PhyloData oPhyloData= new PhyloData(windowStart, windowLength);
    oPhyloData.select();
  }*/
