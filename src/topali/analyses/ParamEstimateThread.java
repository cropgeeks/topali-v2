...
public class ParamEstimateThread extends Thread
{
	...
	public void run()
	{
		...
		{			
			...
			System.out.println("Iter: " + iteration + ": alpha = " + alpha + ", " + cLL);
			...
			System.out.println("Iter: " + iteration + ": ratio = " + ratio + ", " + cLL);
	...
	private double estimateParameter(double d1, double d2, int iteration, boolean isAlpha)
	{
	...
	private double getLikelihood(double d1, double d2, boolean isAlpha)
	{
		...
		if (isAlpha)
			sm = TreeUtilities.getF84SubstitutionModel(alignment, ratio, (d1+d2)/2);
		else
			sm = TreeUtilities.getF84SubstitutionModel(alignment, (d1+d2)/2, alpha);
	...
	private double getLikelihood(SubstitutionModel model)
	{
		...
		likelihood.setTree(new NeighborJoiningTree(
			new AlignmentDistanceMatrix(sitePattern, model)));
	...
	private void calculateAverageDistance()
	{
		SubstitutionModel model =
			TreeUtilities.getF84SubstitutionModel(alignment, ratio, alpha);
	
		AlignmentDistanceMatrix dist =
			new AlignmentDistanceMatrix(new SitePattern(alignment), model);
