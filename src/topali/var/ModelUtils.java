...
public class ModelUtils
{
	...
	public static Model getBestAIC1Model(List<Model> models) {
		...
		for(int i=0; i<models.size(); i++) {
			...
			if(m.isInv())
				df++;
			double aic = MathUtils.calcAIC1(m.getLnl(), df);
			if(aic<best) {
				best = aic;
				index = i;
			}
	...
	public static Model getBestAIC2Model(List<Model> models, int n) {
		double best = Double.MAX_VALUE;
		int index = 0;
		for(int i=0; i<models.size(); i++) {
			Model m = models.get(i);
			...
			if(m.isInv())
				df++;
			double aic = MathUtils.calcAIC2(m.getLnl(), df, n);
			if(aic<best) {
				best = aic;
				index = i;
			}
	...
	public static Model getBestBICModel(List<Model> models, int n) {
		double best = Double.MAX_VALUE;
		int index = 0;
		for(int i=0; i<models.size(); i++) {
			Model m = models.get(i);
			...
			if(m.isInv())
				df++;
			double aic = MathUtils.calcBIC(m.getLnl(), df, n);
			if(aic<best) {
				best = aic;
				index = i;
			}
