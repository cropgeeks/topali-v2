...
void bootstrap::printTreeWithBPvalues(ostream &out, const tree &t, const map<int, MDOUBLE> & v) const{
	recursivlyPrintTreeWithBPvalues(out,t.getRoot(),v);
	out<<";";
}

void bootstrap::recursivlyPrintTreeWithBPvalues(ostream &out, 
									  const tree::nodeP &myNode,
									  const map<int, MDOUBLE> &v) const {
	if (myNode->isLeaf()) {
		out << myNode->name();
		...
		for (int i=0;i<myNode->getNumberOfSons();++i) {
			if (i>0) out <<",";
			recursivlyPrintTreeWithBPvalues(out, myNode->getSon(i),v);
		}
		out <<")";
		if (myNode->isRoot()==false) {
			out<<":"<<myNode->dis2father(); 
			map<int,MDOUBLE>::const_iterator val=v.find(myNode->id());
			if ((val!=v.end()) &&  val->second>0) {
				out << "["<<val->second<<"]";
			} 
		}
	}
...
// 4. get a list of compatible splits
	...
	for (int k=0; k < sortedSplits.size(); ++k) { 
		...
		MDOUBLE support = static_cast<MDOUBLE>(sortedSplits[k].second)/_numTrees;
