...
//------------------------------------------
tree NJalg::computeNJtree(VVdouble distances,const vector<string>& names){
...
tree NJalg::computeNJtree(VVdouble distances,const vector<string>& names, const tree& constriantTree){
	assert(distances.size() == names.size());
	tree resTree = startingTree(names);
...
void NJalg::updateBranchDistance(const VVdouble& distanceTable,
	...
	if (DisI_new<SHORT_LENGTH_VALUE) DisI_new=SHORT_LENGTH_VALUE; // no negative..
	if (DisJ_new<SHORT_LENGTH_VALUE) DisJ_new=SHORT_LENGTH_VALUE; // no negative..
...
void NJalg::update3taxaLevel(VVdouble& distanceTable,Vdouble & r_values,
	...
	if (dis0root<SHORT_LENGTH_VALUE) dis0root=SHORT_LENGTH_VALUE; // no negative..
	if (dis1root<SHORT_LENGTH_VALUE) dis1root=SHORT_LENGTH_VALUE; // no negative..
	if (dis2root<SHORT_LENGTH_VALUE) dis2root=SHORT_LENGTH_VALUE; // no negative..
