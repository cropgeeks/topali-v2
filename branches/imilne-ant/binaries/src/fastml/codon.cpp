...
int codon::fromChar(const string& s, const int pos) const {
	...
	int res =0;
	if ((p1 <0) || (p2 <0) || (p3 <0)) res = -1; // gap.
	else if ((p1 ==15) || (p2 ==15) || (p3 ==15)) res= 64; // unknown.
	else if ((p1 >4) || (p2 >4) || (p3 >4)) return 64; //unknown.
...
codonUtility::replacementType codonUtility::codonReplacement(const int c1, const int c2){
	if (c1 == c2) return codonUtility::sameConon;
	else if (codonUtility::aaOf(c1) == codonUtility::aaOf(c2)) return codonUtility::synonymous;
...
int codonUtility::aaOf (const int c1){
	switch (c1) {
		...
		case 61: case 62: case 63: return -2; //stop
		case 64: case -1: return -1;//unknown or gap
