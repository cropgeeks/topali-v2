...
public class PDMMonitor
{
	...
	public PDMMonitor(File jobDir) throws Exception
	{
		this.jobDir = jobDir;
	}
	...
	public JobStatus getPercentageComplete() throws Exception
	{
		...
		else
		{
			...
			// return 100
			// else
			// Fudge the return so it doesn't appear finished until 105%
			// complete
	...
	float getParallelPercentageComplete() throws Exception
	{
		...
		for (File f : files)
		{
			...
			try
			{
				logger.info("pDir.length = " + pDir.listFiles().length);
			} catch (Exception e)
			{
	...
	public PDM2Result getResult() throws Exception
	{
