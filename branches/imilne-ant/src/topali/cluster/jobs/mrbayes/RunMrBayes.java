...
public class RunMrBayes extends Thread
{
	...
	public RunMrBayes(File jobDir, SequenceSet ss, MBTreeResult result)
	{
		this.jobDir = jobDir;
		this.ss = ss;
		this.result = result;
	...
	private void startThread() throws Exception
	{
		...
		if (result.isRemote)
			MrBayesWebService.runScript(jobDir);
		else
			new MrBayesAnalysis(jobDir).start(LocalJobs.manager);
	}
