...
public class Entropy
{
	...
	// Histogram matrix. Rows represent mean postions in the alignment; colums
	// represent different topologies of the tree.
	...
	// ----------------------------------------------------------
	public boolean getTopologyStrings(String[][] argumentMatrix)
	...
	public void stringMatch(String topoString, int m)
	{
		...
		for (i = 0; i < nRows; i++)
			for (k = 0; k < nCols; k++)
				if (intMatrix[i][k] == 0
						&& topoString.equals(stringMatrix[i][k]))
				{
					intMatrix[i][k] = m;
				}
	}
	...
	public boolean setIntMatrix(File fileName) throws JambeException
	{
		...
		while (true)
		{
				...
				while (oTokenizer.hasMoreTokens())
				{
					if (j > nCols || i > nRows)
					{
						throw new JambeException(
								"EN03: Wrong number of rows or columns for "
										+ "topoArray.");
					}
					topoArrayElement = oTokenizer.nextToken();
					intMatrix[i][j] = this
							.stringToIntegerTranslator(topoArrayElement);
					// Updates Vector stringList
					j++;
				}
				if (j != nCols)
				{
					throw new JambeException("EN04: In file " + fileName
							+ " the number of " + "columns is = " + j
							+ " rather than " + nCols + ".");
				}
				oTokenizer = null;
				i++;
			...
			{
				throw new JambeException("EN05: Unable to read from file "
						+ fileName + ": " + ex);
			}
		}
	...
	// -------------------------------------------------------------
	public boolean setIntMatrix(File fileName, File intFile)
			throws JambeException
	{
		...
		try
		{
			out = new BufferedWriter(new FileWriter(intFile));
		} catch (IOException e)
		{
			throw new JambeException("EN02B: Unable to open file " + intFile
					+ " for writing: " + e);
		}
		...
		// Get the number of different topologies
		nTopos = stringList.size();
		System.out.println("nTOPOS now " + nTopos + ", stringlist= "
				+ stringList.size());
		// Note that this list gets updated above in the call:
		// this.stringToIntegerTranslator(topoArrayElement)
	...
	public int[] numberOfRowsAndCols(File fileName) throws JambeException
	{
		...
		while (true)
		{
			...
			{
				throw new JambeException("EN07: Unable to read from file "
						+ fileName + ": " + ex);
			}
		}
	...
	public void getHistogram() throws JambeException
	{
		...
		{
			throw new JambeException(
					"EN12: Cannot create histogram because nTopos<=0");
		}
	...
	public void showHistogram(File fileName) throws JambeException
	{
		...
		{
			throw new JambeException("EN10: Unable to write to file "
					+ fileName + ": " + ex);
		}
	}
	...
	public void showEntropy(File fileName) throws JambeException
	{
		...
		{
			throw new JambeException("EN11: Unable to write to file "
					+ fileName + ": " + ex);
		}
	}
	...
	private void computeLocalStatistic(int window)
	{
		...
		for (int i = 0; i < hops / 2; i++)
			score += computePQ(histogram[index - i], histogram[index
					- (hops - i - 1)]);
	...
	/*
	 * void computeThreshold(float N) { // What was the maximum number of
	 * non-zero topologies? int max = 0; for (int i = 0; i < maxNonZero.length;
	 * i++) if (maxNonZero[i] > max) max = maxNonZero[i];
	 * 
	 *  // Max per window for (int i = 0; i < threshold.length; i++) { int df =
	 * maxNonZero[i] - 1;
	 * 
	 * if (df == 0) threshold[i] = 0; else { double chi2 =
	 * ChiSquareDistribution.quantile(0.95, df); threshold[i] = (float) chi2 /
	 * (2 * N); } }
	 * 
	 *  // Max per alignment int df = max -1; if (df == 0) df = 1; double chi2 =
	 * ChiSquareDistribution.quantile(0.95, df); float t = (float) chi2 / (2 *
	 * N);
	 * 
	 * for (int i = 0; i < threshold.length; i++) { // threshold[i] = t; }
	 * 
	 *  // Max per histogram df = histogram[0].length -1;
	 * System.out.println("HISTO MAX " + (df+1)); if (df == 0) df = 1; chi2 =
	 * ChiSquareDistribution.quantile(0.95, df); t = (float) chi2 / (2 * N);
	 * 
	 * for (int i = 0; i < threshold.length; i++) { // threshold[i] = t; }
	 * 
	 *  // Max per max-top /* df = getMaxNumberOfTopologies(*** NUMBER OF
	 * SEQUENCES? ***) -1; System.out.println("HISTO MAX " + (df+1)); if (df ==
	 * 0) df = 1; chi2 = ChiSquareDistribution.quantile(0.95, df); t = (float)
	 * chi2 / (2 * N);
	 * 
	 * for (int i = 0; i < threshold.length; i++) { // threshold[i] = t; }
	 */
	 ...
	 * 
	 * 
	 * long getMaxNumberOfTopologies(int n) { long top = F(2 * n - 5);
	 * 
	 * long bot = (long)( Math.pow(2, n-3)) * F(n-3);
	 * 
	 * return top / bot; }
	 * 
	 * long F(long f) { long result = f; for (long i = f-1; i > 0; i--) result *=
	 * i;
	 * 
	 * System.out.println("F: " + f + ": " + result); return result; }
	 */
	...
	public float[] getLocalData()
	{
		 ...
		 * 
		 * int bValue = (int) (Prefs.mcmc_window_size / 2f /
		 * Prefs.mcmc_step_size);
		 * 
		 * for (int i = 0; i < local.length; i++) { int point = i + bValue; if
		 * (point < local.length) actual[i] = local[point]; else actual[i] = 0; }
		 * 
		 * return actual;
		 */
		return local;
	...
	/*
	 * public int getMaxNonZero() { int max = 0; for (int i = 0; i <
	 * maxNonZero.length; i++) if (maxNonZero[i] > max) max = maxNonZero[i];
	 * 
	 * return max; }
	 */
	...
	public void doItAll(int window) throws JambeException
	{
		...
		// this.showHistogram();
		// if (save)
		// this.showHistogram(new File(Prefs.mcmc_scratch,
		// "results_histo.out"));
		this.compEntropy();
		// if (window != -2)
