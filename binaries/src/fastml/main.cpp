...
//	sequenceContainer* sd = clustalFormat::read(f,20);
	sequenceContainer* sd = NULL;
	if (format == 'm') sd=maseFormat::read(f,20);
	else if (format == 'a') sd=clustalFormat::read(f,20);
	else {exit(4343);}
//	sd->changeGapsToMissingData();

	tree t1(treeFile,*sd); // with sequence data
...
//	pijAccelerator *pijAcc1= new trivialAccelerator(probMod1);
	...
	bbAlg bbAlg1(&t1,_s1,bbAlg::both,reportFileName,ancestralSequencesFileName,recalculateExactVal);//computeAgainExactTreshold
...
//	bbAlg bbAlg1(&t1,_s1,bbAlg::max,0);//computeAgainExactTreshold
	...
	MDOUBLE res = bbAlg1.bbReconstructAllPositions();
	time(&time2);
	ctime2 = clock();
