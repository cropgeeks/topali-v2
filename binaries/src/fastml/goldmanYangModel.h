...
public:
	...
	explicit goldmanYangModel(const MDOUBLE inV, const MDOUBLE inK, const bool globalV=true){_globalV = globalV;_v=inV;_k=inK;homogenousFreq();updateQ();};
	explicit goldmanYangModel(const MDOUBLE inV, const MDOUBLE inK, const Vdouble& freq,const bool globalV=true) {_globalV =globalV;_v=inV;_k=inK;_freq=freq;updateQ();};
