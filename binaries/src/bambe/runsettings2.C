...
void RSDoubleArray::parseInput(int& v, int elementGiven[], double ardoubleval[])
{
  ...
  for(pos1=s;*pos1;pos1=pos2,v++) {
	...
	*pos2++ = '\0';
    ...
    else if(((istrstream(pos1,MAX_LINE))>>ardoubleval[v])!=NULL)
      elementGiven[v] = 1;
    else {
      ardoubleval[v] = 0;
      okay = 0;
...
{
  char s[MAX_LINE];
  if(((istrstream(val,MAX_LINE))>>s)==NULL)
    parseError();
  upcaseString(s);
  if(strcmp(s,"TRUE")==0)
    v = 1;
...
RunSettings::RunSettings(istream& f) :
  ...
  mu("mu","2","(0,Inf)","integrate==true"),
  eta("eta","4","(0,Inf)","integrate==true"),
