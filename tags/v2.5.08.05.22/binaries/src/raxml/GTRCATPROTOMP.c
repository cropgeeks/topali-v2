...
double evaluateGTRCATPROT (tree *tr, nodeptr p)
  {
    ...
    double *EIGN = tr->EIGN;
    protlikelivector   *x1, *x1_start, *x2, *x2_start;       
    int cat;
    double term;
    double *left, *right;
...
#pragma omp parallel for private(left, right, diagptable, term, cat, x1, x2) reduction(+ : sum)
	for (i = 0; i < tr->cdta->endsite; i++) 
	  {	    	   
	    cat = cptr[i];	  
	    x1 = &(tr->protTip[tipX1[i]]);
	    x2 = &x2_start[i];
...
#pragma omp parallel for private(left, right, sum, x2)
	    for (i = 0; i < tr->cdta->endsite; i++) 
	      {    
		left = &(tr->protTip[tipX1[i]].v[0]);			       
		x2 = &x2_start[i];
...
#pragma omp parallel for private(left, right, sum, x1, x2)
	    for (i = 0; i < tr->cdta->endsite; i++) 
	      {     
		x1 = &x1_start[i];
		x2 = &x2_start[i];		
...
#pragma omp parallel for private(d1, sum, tmp, inv_Li, dlnLidlz, d2lnLidlz2, l) reduction(+ : dlnLdlz) reduction( + : d2lnLdlz2)
        for (i = 0; i < tr->cdta->endsite; i++) 	
	  {	  	 	   	    
...
{   
...

  {
	...
	ump_x2[20], x1px2, z1, z2, ki;
      ...
      protlikelivector *x1, *x2, *x3, *x1_start, *x2_start, *x3_start;
      ...
      /*#pragma omp parallel for private(ki, d1, d2, EV, left, right)*/
      for(i = 0; i < tr->NumberOfCategories; i++)
	{	
	  ki = rptr[i];
...
#pragma omp parallel for private(cat, EV, left, x3, v, x1px2, l, ump_x1, ump_x2) 	  
	  for (i = 0; i < tr->cdta->endsite; i++) 
	    {		   	    	   	   
	      cat = cptr[i];
	      EV = tr->EV;	       	   
...
#pragma omp parallel for private(cat, EV, left, x1, x2, x3, v, x1px2, l, scale,  ump_x1, ump_x2)
	  for (i = 0; i < tr->cdta->endsite; i++)
	    {			     
	      cat = cptr[i];
	      EV = tr->EV;
