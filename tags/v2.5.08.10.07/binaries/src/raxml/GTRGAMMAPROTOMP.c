...
boolean newviewGTRGAMMAPROT(tree    *tr, nodeptr  p)
{
  ...
  { 	
	...
	{       
	 ...
	 /*TODO worth parallelizing this, problem at *uX2 and *uX1 ?*/
	 /*#pragma omp parallel for private(v, left, right, k, ump_x1_0, l*/
	 for(i = 0; i < 23; i++)
	   {	    	     
	     v = &(tr->protTip[i].v[0]);	     
	     left = left_start;
...
double makenewzGTRGAMMAPROT (tr, p, q, z0, maxiter)
  ...
  { /* makenewz */
    ...
    protgammalikelivector  *x1, *x2, *x2_start, *x1_start;
...
#pragma omp parallel for private(left, right, sumc, sum, l)
	for (i = 0; i < tr->cdta->endsite; i++) 
	  {     
	    left = &(tr->protTip[tipX1[i]].v[0]);
	    right = &(tr->protTip[tipX2[i]].v[0]);      
...
#pragma omp parallel for private(left, right, sumc, sum, l, x2)
	    for (i = 0; i < tr->cdta->endsite; i++) 
	      {     
		left = &(tr->protTip[tipX1[i]].v[0]);		    	  
...
#pragma omp parallel for private(left, right, sumc, sum, l, x1)
		for (i = 0; i < tr->cdta->endsite; i++) 
		  {     
		    right = &(tr->protTip[tipX2[i]].v[0]);		    	      
		...
		x1 = x1_start = (protgammalikelivector*)p->x;
		x2 = x2_start = (protgammalikelivector*)q->x;
#pragma omp parallel for private(left, right, sumc, sum, l, x1, x2)
		for (i = 0; i < tr->cdta->endsite; i++) 
		  {     		    
		    sumc = 0.0;
