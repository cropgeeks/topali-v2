...
MDOUBLE computeLtkCheck(sequenceContainer& sc,
						stochasticProcess& sp,
						const Vdouble * weights,
						const int nodeId,
								   const suffStatGlobalGam& cup,
...
void nwRafImprovIt0(sequenceContainer& sc,
						stochasticProcess& sp,
						const Vdouble * weights,
						tree::nodeP in_nodep,
									   const suffStatGlobalGam& cup,
...
/*	MDOUBLE delta = 0.00001;
	...
	LOG(5,<<"node="<<in_nodep->name()<<endl);
	LOG(5,<<"delta="<<delta<<endl);
	LOG(5,<<"checkik="<<checkik<<endl);
	LOG(5,<<"checkikplus="<<checkikplus<<endl);
	LOG(5,<<"evalderiv1="<<evalderiv1<<endl);
	LOG(5,<<"checkikminus="<<checkikminus<<endl);
	LOG(5,<<"evalderiv2="<<evalderiv2<<endl);
	LOG(5,<<"evalsecDeriv="<<evalsecDeriv<<endl);
	LOG(5,<<"ltk == "<<ltk<<endl);
	LOG(5,<<"dltk == "<<dltk<<endl);
	LOG(5,<<"d2ltk2 == "<<d2ltk2<<endl);
	LOG(5,<<"==============================="<<endl<<endl);
...
void revaluateBranchLengthIt0(sequenceContainer& sc,
						stochasticProcess& sp,
						const Vdouble * weights,
						const tree& et,
						tree::nodeP in_nodep,
	...
	for(int i=0; i<in_nodep->getNumberOfSons(); ++i) {
		revaluateBranchLengthIt0(sc, sp, weights, et, in_nodep->getSon(i), cup, cdown);
	}
...
void revaluateBranchLength(computePijGam& pij0,
						...
						tree::nodeP in_nodep,
									   ...
									   const suffStatGlobalGam& cdown) {
	for(int i=0; i<in_nodep->getNumberOfSons();i++) {
		revaluateBranchLength(pij0,pij1,pij2,sc,sp,weights,et, in_nodep->getSon(i), cup, cdown);
	}
	if (in_nodep == et.getRoot()) return;
...
//	LOG(5,<<" ENTERING2 optimizeBranchLengthPrivate"<<endl);
//	MDOUBLE CHECK = likelihoodComputation::getTreeLikelihood(_et,_pi,NULL);
//	LOG(5,<<"CHECK in brLenOpt::optimizeBranchLengthPrivate= "<<CHECK<<endl);
...
//		LOGDO(1,_et.output(myLog::LogFile(),tree::PHYLIP));
		...
		tree oldT = et;
		if (it==0) {
			revaluateBranchLengthIt0(sc,sp,weights,et,et.getRoot(),cup,cdown);
		}
		else {
			revaluateBranchLength(pij0,pij1,pij2,sc,sp,weights,et,et.getRoot(),cup,cdown);
		}
		pij0.fillPij(et,sp,sc.alphabetSize(),0);
		pij1.fillPij(et,sp,sc.alphabetSize(),1);
		pij2.fillPij(et,sp,sc.alphabetSize(),2);
