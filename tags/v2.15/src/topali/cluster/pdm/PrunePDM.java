...
class PrunePDM
{
		...
		throws Exception
	{
		...
		oEntropy.setIntMatrix(
			new File(wrkDir, "resultsAllTopos.out"),
...
// TODO: is this step required?		
	...
	private void parseTreeDistOutfile()
		throws Exception
	{
	...
	private double getDistanceForGroups(int numGroups)
	{
		...
		System.out.println();
		System.out.println("Best distance for " + numGroups + " is " + bestDistance);
	...
	private void getGroups(Node node, Vector nodes, double cutoffDepth, double currentDepth)
	{
		currentDepth = currentDepth + node.getBranchLength();
	...
	private float[][] computeNewHistogram(float[][] oH)
	{
		...
		System.out.println("NEW HISTOGRAM: " + nH.length + " by " + groups.size());
		
/*		try
		{
			BufferedWriter out = new BufferedWriter(new FileWriter(
				new File(Prefs.mcmc_scratch, "new_histo.txt")));
			
			for (int row = 0; row < nH.length; row++)
			{
				for (int col = 0; col < nH[row].length; col++)
					out.write(d.format(nH[row][col]) + " ");
				out.newLine();
			}
