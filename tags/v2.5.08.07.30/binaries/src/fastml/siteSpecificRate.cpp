...
void computeML_siteSpecificRate(int pos,
								 ...
								 const MDOUBLE tol) {
	LOG(5,<<".");
	MDOUBLE ax=0.0f,bx=maxRate*0.25,cx=maxRate;
...
void computeEB_EXP_siteSpecificRate(int pos,
	...
	Vdouble pGivenR(sp.categories(),0.0);
	MDOUBLE sum=0;
	...
	for (int j=0; j < sp.categories(); ++j) {
		...
		MDOUBLE tmp = pGivenR[j]*sp.rates(j);
		bestRate += tmp;
		Ex2 += (tmp*sp.rates(j));
	}
...
void computeEB_EXP_siteSpecificRate(Vdouble & ratesV,
	...
	computePijGam cpg;
	cpg.fillPij(et,sp,sp.alphabetSize());
	for (int pos=0; pos < sc.seqLen(); ++pos) {
...
void computeEB_EXP_siteSpecificRate(Vdouble & ratesV,
	...
	for (int treeNum=0; treeNum<etVec.size(); ++treeNum) {
		for (int spNum = 0; spNum<spVec.size(); ++spNum) {
            computePijGam cpg;
			cpg.fillPij(etVec[treeNum],*(spVec[spNum]),spVec[spNum]->alphabetSize());
			for (int pos=0; pos < sc.seqLen(); ++pos) {
