...
bbAlg::bbAlg(const tree& et,
	...
	BandBReportAllPos1(reportFileName,et.getInternalNodesNum()*sp.alphabetSize()*sc.seqLen()),
	_et(et), _sp(sp), _sc(sc)
{
	_boundMethod = boundType; 
	_alphabetSize=_sp.alphabetSize();
	_seqLen=_sc.seqLen();
	_cpij.fillPij(_et,_sp,_sp.alphabetSize());
	_bbesavp1 = new bbEvaluateSpecificAV(_et,_sp,_sc,_cpij);
					
	_bbNodeOrderAlg1 = new bbNodeOrderAlg(_et,_sp,_sc,_cpij,computeAgainExactTreshold);
	_bbfindBestAVDynProg1 = new bbfindBestAVDynProg(&_et,&_sp,_sc,&_cpij);
...
MDOUBLE bbAlg::bbReconstructAllPositions(sequenceContainer& res){
	...
	for (int i=0 ; i < _seqLen ; ++i) {
		...
		_bbReport = new BandBReport(_reportFileName,i,_sp.alphabetSize());
		cout<<i+1<<" ";
...
void bbAlg::fillProbOfPosition(const int pos) {
	_pOfPos = likelihoodComputation::getLofPos(pos,_et,_sc,_cpij,_sp);
}
