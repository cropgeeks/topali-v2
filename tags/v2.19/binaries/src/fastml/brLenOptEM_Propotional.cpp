...
MDOUBLE brLenOptEM::optimizeBranchLengthNG_EM_PROP(
	...
	for (i=0; i < cup.size(); ++i) {
		cup[i].allocatePlace(sc[i].seqLen(), sp[i].categories(), et.getNodesNum(), sc[i].alphabetSize());
		cdown[i].allocatePlace(sc[i].seqLen(), sp[i].categories(), et.getNodesNum(),sc[i].alphabetSize());
	}
