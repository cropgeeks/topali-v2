...
boolean newviewGTRGAMMAMULT(tree    *tr, nodeptr  p)
{
  ...
  { 	
    ...
    int  i;
    gammalikelivector   *x1, *x2, *x3, *x1_start, *x2_start, *x3_start;
    ...
    /*#pragma omp parallel for private(EIGN, left, right, i, ki, EV, d1c, d1g, d1t, d2c, d2g, d2t) TODO efficiency of this will probably depend
      on number of models used */
	...
	for(i = 0; i < 4; i++)
	 ...
	 x3  = x3_start = (gammalikelivector*) p->x; 
#pragma omp parallel for private(uX1, uX2, x3, EV, x1px2, model)	 
	 ...
	 x1  = x1_start = (gammalikelivector*)q->x;
	 x3  = x3_start = (gammalikelivector*) p->x; 	        	    	       	
#pragma omp parallel for private(left, uX2, x1, x3, EV, x1px2, ump_x1_0,  ump_x1_1, ump_x1_2,  ump_x1_3, model) 
	 for (i = 0; i < tr->cdta->endsite; i++) 
	   {
	     model = modelptr[i];
	     left = &left_start[model * 48];	     
	...
	while ((! p->x) || (! r->x)) 
	 ...
	 x2  = x2_start = (gammalikelivector*)r->x;
	 x3  = x3_start = (gammalikelivector*) p->x; 	        	    	       	

#pragma omp parallel for private(right, uX1, x2, x3, EV, x1px2, ump_x2_0,  ump_x2_1, ump_x2_2,  ump_x2_3, model)
	 for (i = 0; i < tr->cdta->endsite; i++) 
	   {
	     model = modelptr[i];
	     right = &right_start[model * 48];
     ...
     x2  = x2_start = (gammalikelivector*)r->x;
     x3  = x3_start = (gammalikelivector*) p->x;     
#pragma omp parallel for private(left, right, x1, x2, x3, EV, x1px2, ump_x1_0,  ump_x1_1, ump_x1_2,  ump_x1_3, ump_x2_0,  ump_x2_1,  ump_x2_2,  ump_x2_3, model)
    for (i = 0; i < tr->cdta->endsite; i++) 
      {	
	model = modelptr[i];
...
double evaluateGTRGAMMAMULT (tr, p)
  ...
  { /* evaluate */
    ...
    double *EIGN;
    gammalikelivector   *x1, *x2, *x1_start, *x2_start;
	...
	likelivector *x1;
	 ...
	 x2  = x2_start = (gammalikelivector*)q->x;
#pragma omp parallel for private(term, x1, x2, diagptable, model) reduction(+ : sum)
	for (i = 0; i < tr->cdta->endsite; i++) 
	  {
	    model = modelptr[i];
	...
	x1  = x1_start = (gammalikelivector*) p->x;
#pragma omp parallel for private(term, x1, x2, diagptable, model) reduction(+ : sum)          
	for (i = 0; i < tr->cdta->endsite; i++) 
	  {
	    model = modelptr[i];
...
double makenewzGTRGAMMAMULT (tr, p, q, z0, maxiter)
  ...
  { /* makenewz */
	...
	char *tipX2 = q->tip;
	likelivector *x1, *x2;
#pragma omp parallel for private(x1, x2, sum, sumc, model)	
	for (i = 0; i < tr->cdta->endsite; i++) 
	  { 
	    model = modelptr[i];	    
...
#pragma omp parallel for private(x1, x2, sum, sumc)
		for (i = 0; i < tr->cdta->endsite; i++) 
		  {     
		    x1 = &x1_start[i];
		    x2 = &x2_start[i];
...
boolean newviewGTRGAMMAMULTPARTITION(tree    *tr, nodeptr  p, int model)
{
  ...
  { 	
	...
	*right++ = d2t * *EV++;
	 ...
	 x3  = x3_start = (gammalikelivector*) p->x;
#pragma omp parallel for private(uX1, uX2, x3, EV, x1px2)	 
	 ...
	 x1  = x1_start = (gammalikelivector*)q->x;
	 x3  = x3_start = (gammalikelivector*) p->x; 	        	    	       	
#pragma omp parallel for private(left, uX2, x1, x3, EV, x1px2, ump_x1_0,  ump_x1_1, ump_x1_2,  ump_x1_3)
	 for (i = lower; i < upper; i++) 
	   {	
	     left = left_start;	     
	     x1 = &x1_start[i];	    
...
#pragma omp parallel for private(right, uX1, x2, x3, EV, x1px2, ump_x2_0,  ump_x2_1, ump_x2_2,  ump_x2_3)
	 for (i = lower; i < upper; i++) 
	   {	
	     right = right_start;
	     uX1 = &umpX1[16 * tipX1[i]];			    
	...
	if (ABS(x3->a0) < minlikelihood && ABS(x3->g0) < minlikelihood && ABS(x3->c0) < minlikelihood && ABS(x3->t0) < minlikelihood &&
     ...
     x2  = x2_start = (gammalikelivector*)r->x;
     x3  = x3_start = (gammalikelivector*) p->x;     
#pragma omp parallel for private(left, right, x1, x2, x3, EV, x1px2, ump_x1_0,  ump_x1_1, ump_x1_2,  ump_x1_3, ump_x2_0,  ump_x2_1,  ump_x2_2,  ump_x2_3)
    for (i = lower; i < upper; i++) 
      {	
	left = left_start;
	right = right_start;
  ...
  { /* evaluate */
	...
	*diagptable++ = exp (EIGN[2] * ki * lz);
	...
	likelivector *x1;
	 ...
	 x2  = x2_start = (gammalikelivector*)q->x;
#pragma omp parallel for private(term, x1, x2, diagptable) reduction(+ : sum)
	for (i = lower; i < upper; i++) 
	  {
	    x1 = &(tr->gtrTip[16 * model + tipX1[i]]);	    
	    x2 = &x2_start[i];
