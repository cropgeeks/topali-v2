...
void SetDefaultProposalInfo (void);
...
MrBFlt		relProbs[NUM_MOVE_TYPES];    /* holds relative probabilities of moves         */
MrBFlt		proParam[NUM_MOVE_TYPES][2]; /* holds proposal parameters, like LOCAL tuning  */
int			readWord;					 /* should we read word next ?                    */
PropInfo	parameterInformation[100];   /* information on the parameter proposals        */
...

{
	...
	int			i, j, numProposalMechanisms;
	
	numProposalMechanisms = 43; /* remember to change the upper limit when adding a new proposal mechanism */

	/* relative probs of moves */
	for (i=0; i<NUM_MOVE_TYPES; i++)
		relProbs[i] = proParam[i][0] = proParam[i][1] = 1.0;
		
	for (i=0; i<=numProposalMechanisms; i++) 
		{
		relProbs[i] = parameterInformation[i].proposalRate;
		for (j=0; j<parameterInformation[i].numProposalParams; j++)
			proParam[i][j] = parameterInformation[i].proposalParams[j];
		}
...

{
	...
	echo                 = YES;                      /* flag used by Manual to control printing       */
...

{
	...
	/* check that the log file pointer is closed out */
	if (logFileFp != NULL)
		fclose (logFileFp);
...

{
	...
	/* Move_Omega */
	paramNum = 0;
	strcpy (parameterInformation[paramNum].parameterName[0], "nonsynonymous/synonymous (omega) rate ratio window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 1.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 100.0;

	/* Move_Statefreqs */
	paramNum = 1;
	strcpy (parameterInformation[paramNum].parameterName[0], "state frequencies (statefreq) Dirichlet parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 300.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000.0;

	/* Move_Beta */
	paramNum = 2;
	strcpy (parameterInformation[paramNum].parameterName[0], "morphology 2-state frequencies Beta prior parameter window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 0.5;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 100.0;
	...
	/* Move_Pinvar */
	paramNum = 3;
	strcpy (parameterInformation[paramNum].parameterName[0], "proportion of invariable sites sliding window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 0.1;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 1.0;

	/* Move_Adgamma */
	paramNum = 4;
	strcpy (parameterInformation[paramNum].parameterName[0], "autocorrelated gamma correlation parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 0.5;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 2.0;

	/* Move_SwitchRate */
	paramNum = 5;
	strcpy (parameterInformation[paramNum].parameterName[0], "covarion-like model switching rate window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 1.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 1000.0;

	/* Move_Speciation */
	paramNum = 6;
	strcpy (parameterInformation[paramNum].parameterName[0], "speciation rate window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 1.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_Extinction */
	paramNum = 7;
	strcpy (parameterInformation[paramNum].parameterName[0], "extinction rate window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 1.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_Theta */
	paramNum = 8;
	strcpy (parameterInformation[paramNum].parameterName[0], "neutral coalescence parameter (theta) window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 1.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_Local */
	paramNum = 9;
	strcpy (parameterInformation[paramNum].parameterName[0], "LOCAL (Larget & Simon, 1999) tuning parameter");
	parameterInformation[paramNum].proposalRate = 5.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log(1.1);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000000.0;

	/* Move_BrLen */
	paramNum = 10;
	strcpy (parameterInformation[paramNum].parameterName[0], "branch lengths tuning parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log(2.0);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000000.0;
	...
	/* Move_ExtTBR */
	paramNum = 11;
	strcpy (parameterInformation[paramNum].parameterName[0], "TBR extension probability");
	strcpy (parameterInformation[paramNum].parameterName[1], "TBR tuning parameter");
	parameterInformation[paramNum].proposalRate = 15.0;
	parameterInformation[paramNum].numProposalParams = 2;
	parameterInformation[paramNum].proposalParams[0] = 0.8;
	parameterInformation[paramNum].proposalParams[1] = 2.0 * log(1.6);
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 0.99;
	parameterInformation[paramNum].minimum[1] = 0.0;
	parameterInformation[paramNum].maximum[1] = 10000000.0;

	/* Move_OmegaPur */
	paramNum = 12;
	strcpy (parameterInformation[paramNum].parameterName[0], "purifying selection omega window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 0.1;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_OmegaPos */
	paramNum = 13;
	strcpy (parameterInformation[paramNum].parameterName[0], "positive selection omega window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 1.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_OmegaCat */
	paramNum = 14;
	strcpy (parameterInformation[paramNum].parameterName[0], "omega category window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 0.5;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_LocalClock */
	paramNum = 15;
	strcpy (parameterInformation[paramNum].parameterName[0], "LOCAL (clock) tuning parameter");
	parameterInformation[paramNum].proposalRate = 10.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.1);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	...
	/* Move_SPRClock */
	paramNum = 16;
	strcpy (parameterInformation[paramNum].parameterName[0], "SPR (clock) dirichlet parameter");
	strcpy (parameterInformation[paramNum].parameterName[1], "SPR (clock) exponential parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 2;
	parameterInformation[paramNum].proposalParams[0] = 10.0;
	parameterInformation[paramNum].proposalParams[1] = 10.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000.0;
	parameterInformation[paramNum].minimum[1] = 0.0;
	parameterInformation[paramNum].maximum[1] = 10000.0;

	/* Move_Aamodel */
	paramNum = 17;
	strcpy (parameterInformation[paramNum].parameterName[0], "amino acid model");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 0;
	parameterInformation[paramNum].proposalParams[0] = 0.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;

	/* Move_NNI */
	paramNum = 18;
	strcpy (parameterInformation[paramNum].parameterName[0], "nearest neighbor interchange (NNI)");
	parameterInformation[paramNum].proposalRate = 10.0;
	parameterInformation[paramNum].numProposalParams = 0;
	parameterInformation[paramNum].proposalParams[0] = 0.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_Omega_M */
	paramNum = 19;
	strcpy (parameterInformation[paramNum].parameterName[0], "nonsynonymous/synonmous (omega) tuning parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.5);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000000.0;
	...
	/* Move_Beta_M */
	paramNum = 20;
	strcpy (parameterInformation[paramNum].parameterName[0], "beta tuning parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.5);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_GammaShape_M */
	paramNum = 21;
	strcpy (parameterInformation[paramNum].parameterName[0], "gamma shape tuning parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.5);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_SwitchRate_M */
	paramNum = 22;
	strcpy (parameterInformation[paramNum].parameterName[0], "covarion model switching rate tuning parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.5);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_Speciation_M */
	paramNum = 23;
	strcpy (parameterInformation[paramNum].parameterName[0], "speciation rate tuning parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.5);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_Extinction_M */
	paramNum = 24;
	strcpy (parameterInformation[paramNum].parameterName[0], "extinction rate tuning parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.5);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_ClockRate */
	paramNum = 25;
	strcpy (parameterInformation[paramNum].parameterName[0], "clock rate tuning parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.1);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_NNI_Hetero */
	paramNum = 26;
	strcpy (parameterInformation[paramNum].parameterName[0], "NNI (heterogeneous) tuning parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.6);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000000.0;
	...
	/* Move_NodeSlider */
	paramNum = 27;
	strcpy (parameterInformation[paramNum].parameterName[0], "node slider tuning parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.1);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_Growth */
	paramNum = 28;
	strcpy (parameterInformation[paramNum].parameterName[0], "population growth window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 5.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = -10000.0;
	parameterInformation[paramNum].maximum[0] = 10000.0;

	/* Move_OmegaNeu */
	paramNum = 29;
	strcpy (parameterInformation[paramNum].parameterName[0], "neutral selection omega window size");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 0.5;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_Tratio_Dir */
	paramNum = 30;
	strcpy (parameterInformation[paramNum].parameterName[0], "transtion/transversion rate (tratio) beta parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 50.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000.0;
	...
	/* Move_Revmat_Dir */
	paramNum = 31;
	strcpy (parameterInformation[paramNum].parameterName[0], "GTR rate (revmat) dirichlet parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 500.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000.0;
	...
	/* Move_OmegaBeta_M */
	paramNum = 32;
	strcpy (parameterInformation[paramNum].parameterName[0], "M10 beta distribution tuning parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log(1.1);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_OmegaGamma_M */
	paramNum = 33;
	strcpy (parameterInformation[paramNum].parameterName[0], "M10 gamma distribution tuning parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log(1.1);
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	paramNum = 34;
	strcpy (parameterInformation[paramNum].parameterName[0], "node slider tuning parameter");
	strcpy (parameterInformation[paramNum].parameterName[1], "node slider exponential parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 2;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log(1.1);
	parameterInformation[paramNum].proposalParams[1] = 10.0;
	...
	/* Move_BiasedSpr */
	paramNum = 35;
	strcpy (parameterInformation[paramNum].parameterName[0], "biased SPR tuning parameter");
	strcpy (parameterInformation[paramNum].parameterName[1], "biased SPR warp parameter");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 2;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log(1.1);
	parameterInformation[paramNum].proposalParams[1] = 1.0;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000000.0;
	parameterInformation[paramNum].minimum[1] = 0.0;
	parameterInformation[paramNum].maximum[1] = 10000.0;
	...
	/* Move_RateMult_Dir */
	paramNum = 36;
	strcpy (parameterInformation[paramNum].parameterName[0], "site specific rate (ratemult) dirichlet parameter");
	parameterInformation[paramNum].proposalRate = 1.0;
	parameterInformation[paramNum].numProposalParams = 1;
	parameterInformation[paramNum].proposalParams[0] = 500.0;
	parameterInformation[paramNum].proposalParams[1] = 0.0;
	...
	/* Move_SPRClock */
	paramNum = 37;
	strcpy (parameterInformation[paramNum].parameterName[0], "extending SPR (clock) tuning parameter");
	strcpy (parameterInformation[paramNum].parameterName[1], "extending SPR (clock) extension prob.");
	parameterInformation[paramNum].proposalRate = 10.0;
	parameterInformation[paramNum].numProposalParams = 2;
	parameterInformation[paramNum].proposalParams[0] = 2.0 * log (1.1);
	parameterInformation[paramNum].proposalParams[1] = 0.8;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000.0;
	parameterInformation[paramNum].minimum[1] = 0.0;
	parameterInformation[paramNum].maximum[1] = 10000.0;
	...
	/* Move_ParsEraser1 */
	paramNum = 38;
	strcpy (parameterInformation[paramNum].parameterName[0], "parsimony eraser 1 alphaPi");
	strcpy (parameterInformation[paramNum].parameterName[1], "parsimony eraser 1 warp factor");
	parameterInformation[paramNum].proposalRate = 0.0;
	parameterInformation[paramNum].numProposalParams = 2;
	parameterInformation[paramNum].proposalParams[0] = 0.5;
	parameterInformation[paramNum].proposalParams[1] = 0.1;
	parameterInformation[paramNum].minimum[0] = 0.0;
	parameterInformation[paramNum].maximum[0] = 10000.0;
	parameterInformation[paramNum].minimum[1] = 0.0;
	parameterInformation[paramNum].maximum[1] = 10000.0;
