...
void bbNodeOrderAlg::putBack(tree::nodeP& node2check,const MDOUBLE & exactVal) {
...
void bbNodeOrderAlg::rankRemainingNodesAccordingToTheirMarginalProb(
	...
	typedef pair<MDOUBLE,tree::nodeP> sortedElement;
...
MDOUBLE bbNodeOrderAlg::getNodeHighestMarginal(const tree::nodeP& inNodeP) {
	MDOUBLE highestProb =0.0;
...
vector<int> bbNodeOrderAlg::findBestOrderInNode(const tree::nodeP node2check,
	...
	typedef pair<MDOUBLE,int> sortedElement; // (marginal, letter)
	vector<sortedElement> sortVec;
	int i,s;
