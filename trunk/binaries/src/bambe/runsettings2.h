...
class RSInt : public RunSetting {
 public:
  ...
  void parse(const RunSettings &rs) {
    if(((istrstream(val,MAX_LINE))>>v)==NULL)
      parseError();
    checkIntBounds(v);
    checkRestriction();
  }
...
class RunSettings {
 public:
  ...
  int getIntegrate() const { return integrate.v; }
  double getAlpha() const { return mu.v / eta.v; }
  double getLambda() const { return 1.0 / eta.v; }
 ...
 private:
  ...
  RSBoolean	 integrate;			// whether or not to integrate out branch lengths
  RSDouble	 mu;				// Loglikelihood constant
  RSDouble	 eta;				// Loglikelihood constant
