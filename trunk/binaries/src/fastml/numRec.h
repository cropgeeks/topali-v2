...
MDOUBLE dbrent(MDOUBLE ax, MDOUBLE bx, MDOUBLE cx, regF f,
	...
	x=w=v=bx;
	fw=fv=fx=f(x);
	assert(fv==fv);// through an exception if answer is nan = not a number.
	dw=dv=dx=df(x);
	for (iter=1;iter<=ITMAX;iter++) {
		xm=0.5*(a+b);
#ifdef VERBOS
		if (iter>10) LOG(5,<<"iteration: "<<iter<<" xm = "<<xm<<" x= "<<x<<" a= "<<a<<" b= "<<b<<" fx= "<<fx<<endl);
#endif
		tol1=tol*fabs(x)+ZEPS;
		tol2=2.0*tol1;//LOG(5,<<"tol1 = "<<tol1<<" tol 2 = "<<tol2<<endl);
		if (fabs(x-xm) <= (tol2-0.5*(b-a))) {
			*xmin=x;
			return fx;
		}
		...
		if (fabs(d) >= tol1) {
			...
			u=x+SIGN(tol1,d); //LOG(5,<<"x = "<<x<<endl; cerr<<"tol1 = "<<tol1<<endl;cerr<<" d= "<<d<<endl;cerr<<" u= "<<u<<endl);
