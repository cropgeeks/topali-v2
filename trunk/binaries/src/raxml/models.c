...
void baseFrequenciesGTR(rawdata *rdta, cruncheddata *cdta, tree *tr, analdef *adef)
{
  ...
  if(!adef->useMultipleModel)
      ...
      for (k = 1; k <= 8; k++) 
	{
	  ...
	  tr->frequencies[0] = freqa;
	  tr->frequencies[1] = freqc;
	  tr->frequencies[2] = freqg;
	  tr->frequencies[3] = freqt;
	}
    }
  else
    {
      ...
      for(model = 0; model < tr->NumberOfModels; model++)
	{
	  ...
	  if(adef->model == M_PROTCAT || adef->model == M_PROTGAMMA)
		...
		{	     	   	    	      			
	      ...
	      for(l = 0; l < 20; l++) 
		{	    
		  tr->frequencies[model * 20 + l] = pfreqs[l];		 
		}	     
	    }
	  else
	    {
	      ...
	      for (k = 1; k <= 8; k++) 
		{
	      ...
	      tr->frequencies[model * 4] = freqa;
	      tr->frequencies[model * 4 + 1] = freqc;
	      tr->frequencies[model * 4 + 2] = freqg;
	      tr->frequencies[model * 4 + 3] = freqt;
	    }
...
void initProtMat(tree *tr, int model, double f[20], analdef *adef, int proteinMatrix)
{ 
  double q[20][20];
  double daa[400], max, temp;
  int i, j, r;
  double *initialRates = &(tr->initialRates[model * tr->numberOfRates]);
...
void initReversibleGTR(rawdata *rdta, cruncheddata *cdta, tree *tr, analdef *adef, int model)
{ 
  if(adef->model == M_PROTCAT || adef->model == M_PROTGAMMA)
    {
      double r[20][20], a[20][20], *initialRates = &(tr->initialRates[model * 190]), 
	f[20], e[20], d[20];
      int i, j, k, m, jj, code, l;
      ...
      if(adef->useMultipleModel)
	{
	  if(adef->proteinMatrix == GTR)
	    ...
	    {	     
	      initProtMat(tr, model, f, adef, adef->protModels[model]);
	      
	      if(adef->protFreqs[model])
		{	     	      
		  for(l = 0; l < 20; l++)		
		    f[l] = tr->frequencies[model * 20 + l];			       
		}
	      else
		{	     
		  for(l = 0; l < 20; l++)		
		    tr->frequencies[model * 20 + l] = f[l];
		}
	    }
	}
      else
	{
	  if(adef->proteinMatrix == GTR)
	    ...
	    {	  
	      initProtMat(tr, model, f, adef, adef->proteinMatrix);
      ...
      for (j = 0; j < 20; j++) 
	{
      ...
      /* TODO MULT SOSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS*/

      tr->fracchanges[model] = 0;
      tr->fracchange = 0;

      for (j = 0; j< 20; j++)
	for (k = 0; k< 20; k++)
	  tr->fracchanges[model] += f[j] * r[j][k] * f[k];
      ...
      for (i=0; i< 20; i++) 
	{
      ...
      for(l = 1; l < 20; l++)
	tr->EIGN[model * 19 + (l - 1)] = EIGN[l]; 
      
      eptr = &(tr->EV[model * 400]);
	...
	for(j = 0; j < 20; j++)      	    
	  *eptr++ = EIGV[i][j];	             
      ...
      for(i = 0; i < 20; i++)
	for(j = 1; j < 20; j++)
	  tr->EI[model * 380 + i * 19 + (j - 1)] = tr->EV[model * 400 + i * 20 + j] * invfreq[i];        	             
    
      for(i=0; i < 23; i++)
	{     
	  for(j = 0; j < 20; j++)
	    tr->protTip[model * 23 + i].v[j] = 0.0; 
	  
	  if(i < 20)
	    {	  
	      for (j = 0; j < 20; j++) 	    
		tr->protTip[model * 23 + i].v[j] += EIGV[i][j];	  
	    }
	  else
	    {
	      if(i == 20)
		{
		  for (j = 0; j < 20; j++) 	    
		    {
		      tr->protTip[model * 23 + i].v[j] += EIGV[2][j] + EIGV[3][j];
		    }
		}
	      else
		{
		  if(i == 21)
		    {
		      for (j = 0; j < 20; j++) 	    
			{
			  tr->protTip[model * 23 + i].v[j] += EIGV[5][j] + EIGV[6][j];
			}
		    }
		  else
		    {
			  ...
			  for (j = 0; j < 20; j++) 	    
			    {
			      for(k = 0; k < 20; k++)
				{
				  tr->protTip[model * 23 + i].v[j] += EIGV[k][j];	  			      
				}
			    }
			}
		    }
    ...
    {
      double r[4][4], a[4][4], *initialRates = &(tr->initialRates[model * 5]), 
	f[4], e[4], d[4];
      int i, j, k, m, jj, code;
      double invfreq[4], EIGN[4], EIGV[4][4]; 
      double *eptr;
    
      f[0] = tr->frequencies[model * 4];
      f[1] = tr->frequencies[model * 4 + 1];
      f[2] = tr->frequencies[model * 4 + 2];
      f[3] = tr->frequencies[model * 4 + 3];
      ...
      for (i=0; i< 4; i++) 
	{
      ...
      tr->EIGN[model * 3] = EIGN[1];
      tr->EIGN[model * 3 + 1] = EIGN[2];
      tr->EIGN[model * 3 + 2] = EIGN[3];        
	...
	for(j = 0; j < 4; j++) 	  	  
	  *eptr++ = EIGV[i][j];	    	   
      ...
      tr->EI[model * 12]     = tr->EV[model * 16 + 1] * invfreq[0];
      tr->EI[model * 12 + 1] = tr->EV[model * 16 + 2] * invfreq[0];
      tr->EI[model * 12 + 2] = tr->EV[model * 16 + 3] * invfreq[0];
      
      tr->EI[model * 12 + 3] = tr->EV[model * 16 + 5] * invfreq[1];
      tr->EI[model * 12 + 4] = tr->EV[model * 16 + 6] * invfreq[1];
      tr->EI[model * 12 + 5] = tr->EV[model * 16 + 7] * invfreq[1];
      
      tr->EI[model * 12 + 6] = tr->EV[model * 16 + 9]  * invfreq[2];
      tr->EI[model * 12 + 7] = tr->EV[model * 16 + 10] * invfreq[2];
      tr->EI[model * 12 + 8] = tr->EV[model * 16 + 11] * invfreq[2];
      
      tr->EI[model * 12 + 9]  = tr->EV[model * 16 + 13] * invfreq[3];
      tr->EI[model * 12 + 10] = tr->EV[model * 16 + 14] * invfreq[3];
      tr->EI[model * 12 + 11] = tr->EV[model * 16 + 15] * invfreq[3];
           
      for(i=0; i < 16; i++)
	{
	  code = i;
	  tr->gtrTip[model * 16 + i].a = 0;
	  tr->gtrTip[model * 16 + i].c = 0;
	  tr->gtrTip[model * 16 + i].g = 0;
	  tr->gtrTip[model * 16 + i].t = 0;
	  if(i > 0)
	    {		      
	      for (j = 0; j < 4; j++) 
		{	    
		  if ((code >> j) & 1) 
		    {
		      int jj = "0123"[j] - '0';				 		     
		      tr->gtrTip[model * 16 + i].a += EIGV[jj][0];
		      tr->gtrTip[model * 16 + i].c += EIGV[jj][1];
		      tr->gtrTip[model * 16 + i].g += EIGV[jj][2];
		      tr->gtrTip[model * 16 + i].t += EIGV[jj][3];
		    }	     		  
		}	    
	    }     
	}
...
void makeGammaCats(tree *tr, int model)
{
  int i, 
    K = tr->gammaCategories;
  double gap05, t, factor, lnga1, alfa, beta;
  double 
    *gammaProbs = (double *)malloc(K * sizeof(double));

  alfa = beta = tr->alphas[model];

  if(alfa < ALPHA_MIN)
    {
      printf("GAMMA_CATS alfa smaller ALPHA_MIN\n");
      exit(-1);
    }
  ...
  if(K==1) 
   ...
   tr->gammaRates[model * K] = gammaProbs[0]*factor;

   tr->gammaRates[model * K + (K-1)] = (1 - gammaProbs[K-2])*factor;

   for (i=1; i<K-1; i++)  
     tr->gammaRates[model * K + i] = (gammaProbs[i] - gammaProbs[i-1])*factor;    
...
{  
  ...
  switch(adef->model)
    ...
    case M_PROTGAMMA:      
       if(adef->useMultipleModel)
	{
	  newview = newviewGTRGAMMAPROTMULT;
	  makenewz = makenewzGTRGAMMAPROTMULT;
	  evaluate = evaluateGTRGAMMAPROTMULT;

	  evaluatePartition = evaluateGTRGAMMAPROTMULTPARTITION;
	  newviewPartition = newviewGTRGAMMAPROTMULTPARTITION;	
	}
      else
	{
	  newview = newviewGTRGAMMAPROT;
	  makenewz = makenewzGTRGAMMAPROT;
	  evaluate = evaluateGTRGAMMAPROT;	  	 
	}
	...
	{	
	  tr->alphas[model] = 1.0;
	  initReversibleGTR(rdta, cdta, tr, adef, model);     
    ...
    case M_PROTCAT:   
      if(adef->useMultipleModel)
	{
	  newview = newviewGTRCATPROTMULT; 
	  makenewz = makenewzGTRCATPROTMULT; 
	  evaluate = evaluateGTRCATPROTMULT;	 
	  
	  evaluatePartial = evaluatePartialGTRCATPROTMULT;
	}
      else
	{
	  newview = newviewGTRCATPROT;
	  makenewz = makenewzGTRCATPROT;
	  evaluate = evaluateGTRCATPROT;
      
	  evaluatePartial = evaluatePartialGTRCATPROT;
	}
      ...
      if(adef->useMultipleModel)
	{
	  ...
	  newview = newviewGTRGAMMAMULT;
	  makenewz = makenewzGTRGAMMAMULT;
	  evaluate = evaluateGTRGAMMAMULT;

	  evaluatePartition = evaluateGTRGAMMAMULTPARTITION;
	  newviewPartition = newviewGTRGAMMAMULTPARTITION;
	}
      else
	{	  
    ...
    case M_GTRCAT:
      if(adef->useMultipleModel)
	{	
	  newview           = newviewGTRCATMULT; 
	  makenewz          = makenewzGTRCATMULT; 
	  evaluate          = evaluateGTRCATMULT;

	  evaluatePartial   = evaluatePartialGTRCATMULT;
	  
	  evaluatePartition = evaluateGTRCATMULTPARTITION;
	  newviewPartition  = newviewPARTITIONGTRCATMULT;
	}
      else
	{      
      ...
      for (j = 0; j < tr->cdta->endsite; j++) 
	{
      ...
      for(model = 0; model < tr->NumberOfModels; model++)
	{
	  tr->alphas[model] = 1.0; /* GAMMA-APPROX */
	  makeGammaCats(tr, model); /*GAMMA-APPROX */
	  initReversibleGTR(rdta, cdta, tr, adef, model);            
	}
      break;    
    }
