...
class FileNexus extends FileGeneric
{
		...
		try
		{
			...
			while (str != null)
			{
				...
				if (!inComment)
				{
					...
					if (index != -1)
						length = Integer.parseInt(str.substring(index + 6, str
								.indexOf(";", index + 6)));
			...
			boolean useSafeNames) throws IOException
	{
		...
		// Comments
		/*
		 * out.write("[NEXUS alignment file generated by TOPALi -
		 * www.bioss.sari.ac.uk/~iainm/topali" + Prefs.sepL); out.write("
		 * Alignment: " + ss.getName() + ", partition from " + (start+1) + " to " +
		 * end + nl + nl);
		 * 
		 * out.write("" + "This file can be input into SPLITSTREE to produce a
		 * graphical check for" + nl + "evidence of recombination among the
		 * sequences." + nl + nl + "This file can also be input into PAUP* for
		 * further phylogenetic analysis." + nl);
		 *  // Are we saving the entire alignment - if so, then include summary
		 * if (start == 0 && end == ss.getLength() && index.length ==
		 * ss.getSize()) out.write(nl + SummaryDialog.getSummaryInformation(nl) +
		 * nl);
		 * 
		 * out.write(nl + "]" + nl + nl);
		 */
		...
		// begin taxa
		out.write("begin taxa;" + nl + "  dimensions ntax=" + index.length
				+ ";" + nl);
