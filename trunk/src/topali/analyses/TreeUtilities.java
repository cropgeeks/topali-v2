...
{
	// F84 Model
	public static SubstitutionModel getF84SubstitutionModel(
		Alignment a, double ratio, double alpha)
	{
		double[] freqs = AlignmentUtils.estimateFrequencies(a);

		// ratio is ts/tv ratio
		double[] params = { ratio, freqs[0], freqs[1], freqs[2], freqs[3] };

		RateMatrix rmat = RateMatrixUtils.getInstance(
			a.getDataType().getTypeID(),
			NucleotideModelID.F84,
			params,
			freqs);
	...
	public static SubstitutionModel getJCSubstitutionModel(Alignment a)
	{
		...
		double[] params = { 0.5, freqs[0], freqs[1], freqs[2], freqs[3] };
	...
	// JC distance matrix
	public static JukesCantorDistanceMatrix getJukesCantorDistanceMatrix(Alignment a)
	{
