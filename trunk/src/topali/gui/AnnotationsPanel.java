...
class AnnotationsPanel extends JPanel
{
	...
	private int canW, charW;
	
	AnnotationsPanel(AlignmentPanel panel, AlignmentData data)
	{
		this.data = data;
		this.panel = panel;
		seqList = panel.getListPanel();
		
		cdsCanvas = new CDSCanvas();
		cdsSP = new JScrollPane(cdsCanvas);
		cdsSP.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		cdsSP.getHorizontalScrollBar().setModel(
			panel.hBar.getModel());
	...
	void test()
	{
		System.out.println("aBar.value="+cdsSP.getHorizontalScrollBar().getValue());
...
//		System.out.println("anno.max="+cdsSP.getHorizontalScrollBar().getMaximum());
	...
	void setSizes(int canW, int charW, int seqListWidth)
	{
		this.canW = canW;
		this.charW = charW;
	...
	private class CDSCanvas extends JPanel
	{
		...
		class CanvasMouseMotionListener extends java.awt.event.MouseMotionAdapter
		{
			String str = null;
