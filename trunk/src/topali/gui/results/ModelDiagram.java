...
public class ModelDiagram extends JPanel
{
	...
	public ModelDiagram(DNAModel model) {
		setModel(model);
	}

	public void setModel(DNAModel model) {
		this.model = model;
		this.subRates = model.getSubRates().clone();
		this.baseFreqs = model.getBaseFreqs().clone();
		this.subRateGroups = model.getSubRateGroups();
		this.baseFreqGroups = model.getBaseFreqGroups();
		for(int i=0; i<this.subRates.length; i++)
			this.subRates[i] = Math.sqrt(this.subRates[i]);
		
		for(double d: this.subRates) {
			if(d>maxRate)
				maxRate = d;
		}
		for(double d: model.getBaseFreqs()) {
			if(d>maxFreq)
				maxFreq = d;
		}
	}
	...
	public void paint(Graphics g)
	{
		...
		try
		{
			...
			g2d.setColor(getSubRateColor(subRateGroups[0]));
			int r = (int)((subRates[0]/maxRate)*l);
			p = mp[3][1];
			g2d.fillOval(p.x-(r/2), p.y-(r/2), r, r);
			
			g2d.setColor(getSubRateColor(subRateGroups[1]));
			r = (int)((subRates[1]/maxRate)*l);
			p = mp[3][2];
			g2d.fillOval(p.x-(r/2), p.y-(r/2), r, r);
			
			g2d.setColor(getSubRateColor(subRateGroups[2]));
			r = (int)((subRates[2]/maxRate)*l);
			p = mp[3][3];
			g2d.fillOval(p.x-(r/2), p.y-(r/2), r, r);
			
			g2d.setColor(getSubRateColor(subRateGroups[3]));
			r = (int)((subRates[3]/maxRate)*l);
			p = mp[4][2];
			g2d.fillOval(p.x-(r/2), p.y-(r/2), r, r);
			
			g2d.setColor(getSubRateColor(subRateGroups[4]));
			r = (int)((subRates[4]/maxRate)*l);
			p = mp[4][3];
			g2d.fillOval(p.x-(r/2), p.y-(r/2), r, r);
			
			g2d.setColor(getSubRateColor(subRateGroups[5]));
			r = (int)((subRates[5]/maxRate)*l);
			p = mp[5][3];
			g2d.fillOval(p.x-(r/2), p.y-(r/2), r, r);
	...
	public static void main(String[] args) {
		...
		ModelDiagram d = new ModelDiagram(mod);
