...
class MosaicPanel extends JPanel implements ActionListener
{
	...
	MosaicPanel(AlignmentData data, HMMResult iResult)
	{
		...
		String[] columnNames =
		{ "Breakpoint", "Topology Before Breakpoint" };
	...
	private Vector<BreakPoint> checkData(boolean write)
	{
		...
		for (int i = 0; i < model.getRowCount(); i++)
		{
			try
			{
				...
				// int tp = Integer.parseInt((String) model.getValueAt(i, 1)) -
				// 1;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (e.getSource() == bReset)
		{
			if (MsgBox
					.yesno(
							"This will reset and regenerate the current "
	...
	private void populateTable(Vector<BreakPoint> breakpoints)
	{
		...
		} else
		{
			for (int i = 0; i < breakpoints.size(); i++)
			{
				...
				String[] bp =
				{ "" + point.breakpoint, "" + getTopology(point.topology + 1) };
				model.addRow(bp);
	...
	private void populateTableFromTDS()
	{
		...
		{
			String[] endBP =
			{ "" + length, getTopology(1) };
			model.addRow(endBP);
		}

		table.setEditingRow(table.getRowCount() - 1);
		table.getCellEditor(table.getRowCount() - 1, 0).addCellEditorListener(
				listen1);
		table.getCellEditor(table.getRowCount() - 1, 1).addCellEditorListener(
				listen1);
	}
	...
	void setBreakPointArray(int[][] bpArray)
	{
		...
		for (int i = 0; i < bpArray.length; i++)
			breakpoints
					.addElement(new BreakPoint(bpArray[i][0], bpArray[i][1]));
