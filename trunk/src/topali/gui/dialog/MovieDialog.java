...
public class MovieDialog extends JDialog implements ActionListener, ChangeListener
{
	private static final int STOPPED = 0;
	private static final int PLAYING = 1;

	private WinMain winMain = null;
	...
	private void createControls()
	{
		...
		canPanel.setBorder(BorderFactory.createCompoundBorder(
		...
		checkCurrent = new JCheckBox(Text.GuiDiag.getString("MovieDialog.gui02"),
			Prefs.gui_movie_current);
		checkCurrent.addActionListener(this);
		checkCurrent.setMnemonic(KeyEvent.VK_O);
		checkCurrent.setToolTipText(Text.GuiDiag.getString("MovieDialog.gui03"));
		
		checkCircular = new JCheckBox(Text.GuiDiag.getString("MovieDialog.gui04"),
			Prefs.gui_movie_circular);
		checkCircular.addActionListener(this);
		checkCircular.setMnemonic(KeyEvent.VK_C);
		checkCircular.setToolTipText(Text.GuiDiag.getString("MovieDialog.gui05"));
...
//		toolbar.add(new JLabel(" "));
	...
	public void stateChanged(ChangeEvent e)
	{
		...
		try { createTree(sNuc, eNuc); }
		catch (Exception e1) { currentFrame--; }
		
		Object[] args = { slider.getValue(), frames, sNuc, eNuc };
		sliderLabel.setText(Text.format(
			Text.GuiDiag.getString("MovieDialog.gui07"), args));
	}
...
//		tree = tc.createTree(TreeCreator.JC_NJ);
	...
	private void setFrames()
	{
		...
		if (Prefs.gui_movie_window < 2 || Prefs.gui_movie_window > ss.getLength())
			Prefs.gui_movie_window = 2;
		if (Prefs.gui_movie_step < 1 || Prefs.gui_movie_step > ss.getLength())
			Prefs.gui_movie_step = 1;
	
		frames = ((ss.getLength() - Prefs.gui_movie_window) /
			Prefs.gui_movie_step) + 1;
	...
	class TreePlayer implements Runnable
	{
		public void run()
		{
			...
			{
				try { Thread.sleep(Prefs.gui_movie_delay); }
	...
	class AdvancedPanel extends JPanel implements ChangeListener
	{
		...
		AdvancedPanel()
		{			
			winModel = new SpinnerNumberModel(
				Prefs.gui_movie_window, 2, ss.getLength(), 1);
			stepModel = new SpinnerNumberModel(
				Prefs.gui_movie_step, 1, ss.getLength(), 1);
			delayModel = new SpinnerNumberModel(
				Prefs.gui_movie_delay, 5, 5000, 5);
			...
			((JSpinner.NumberEditor)delaySpin.getEditor()).getTextField()
			...
			label3.setLabelFor(((JSpinner.NumberEditor)delaySpin.getEditor()).getTextField());
