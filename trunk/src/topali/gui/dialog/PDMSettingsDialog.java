...
//	private ExtraPanel extraPanel;
	...
	public PDMSettingsDialog(WinMain winMain, AlignmentData data, PDMResult iResult)
	{
		super(winMain, "Probabilistic Divergence Measures - Confirm Settings", true);
		this.data = data;
...
//				return;
			...
			createPDMObject((e.getModifiers() & e.CTRL_MASK) == 0);
			setVisible(false);
		}
...
//			lSample = new JLabel("Sample-interval:");
			...
			// Newick format
			String[] v5 = { "true", "false" };
//			cNewick = new JComboBox(v4);
//			cNewick.setSelectedItem(Prefs.pdm_newick_format);
//			cNewick.setToolTipText("Format of output tree, false implies bambe format");
...
//			cSingleKap.setToolTipText("Use separate kappa values for all site categories");
...
//			lGAM = new JLabel("Initial-gamma:");
			...
			cParam.setToolTipText("Number of tree updates after each parameter update");
			lParam = new JLabel("Parameter-update-interval:");
			
			// Update-kappa
//			cuKap = new JComboBox(v4);
//			cuKap.setSelectedItem(Prefs.pdm_update_kappa);
//			cuKap.setToolTipText("Transition/tranversion bias parameters, not used");
...
//			cuTTP.setToolTipText("Transition/tranversion bias parameters for TN93");
//			luTTP = new JLabel("Update-ttp:");
			
			// Update-kappa
//			cuGAM = new JComboBox(v4);
//			cuGAM.setSelectedItem(Prefs.pdm_update_gamma);
//			cuGAM.setToolTipText("Transition/tranversion bias parameters for TN93");
//			luGAM = new JLabel("Update-gamma:");
			
			// Tune-interval
			ctInt = new JTextField("" + Prefs.pdm_tune_interval);
			ctInt.setToolTipText("Interval to adapt to global-tune during burn-in");
...
//			ltPi = new JLabel("Pi-tune:");
			...
			// Kappa-tune
//			ctKap = new JTextField("" + Prefs.d5.format(Prefs.pdm_kappa_tune));
...
//			ltTTP = new JLabel("Ttp-tune:");
			...
			// Gamma-tune
//			ctGam = new JTextField("" + Prefs.d5.format(Prefs.pdm_gamma_tune));
...
//			ltGam = new JLabel("Gamma-tune:");
			...
			// Intitial-tree-type
//			String[] v6 = { "random", "upgma", "neighbor-joining", "newick", "bambe" };
...
//			cHeight = new JTextField("" + Prefs.d5.format(Prefs.pdm_max_initial_tree_height));
...
//			cTreeFile.setToolTipText("Name of file with initial tree (newick or bambe)");
...
//			layout.add(cTreeFile, 1, 39, 1, 1, new Insets(5, 5, 10, 10));
		...
		boolean saveSettings()
		{
			...
			try { Prefs.pdm_burn = Integer.parseInt(cBurn.getText()); }
			catch (Exception e)	{
				return error("Burn-in time: positive integer expected.", cBurn);
			}
			
			try { Prefs.pdm_cycles = Integer.parseInt(cCycles.getText()); }
			catch (Exception e)	{
...
//			try { Prefs.pdm_sample_interval = Integer.parseInt(cSample.getText()); }
//			catch (Exception e)	{
//				return error("Sample-interval: positive integer expected.", cBurn);
//			}
...
//			Prefs.pdm_initial_gamma = cGAM.getText();
			...
			try { Prefs.pdm_parameter_update_interval = Integer.parseInt(cParam.getText()); }
			catch (Exception e)	{
				return error("Parameter-update-interval: positive integer expected.", cParam);
			}
...
//			Prefs.pdm_update_gamma = (String) cuGAM.getSelectedItem();
			...
			try { Prefs.pdm_tune_interval = Integer.parseInt(ctInt.getText()); }
			catch (Exception e)	{
				return error("Tune-interval: positive integer expected.", ctInt);
			}
			
			try { Prefs.pdm_global_tune = Prefs.d5.parse(ctGlo.getText()).floatValue(); }
			catch (Exception e)	{
				return error("Global-tune: positive number expected.", ctInt);
			}
			
			try { Prefs.pdm_local_tune = Prefs.d5.parse(ctLoc.getText()).floatValue(); }
			catch (Exception e)	{
				return error("Local-tune: positive number expected.", ctLoc);
			}
			
			try { Prefs.pdm_theta_tune = Prefs.d5.parse(ctThe.getText()).floatValue(); }
			catch (Exception e)	{
				return error("Theta-tune: positive number expected.", ctThe);
			}
			
//			try { Prefs.pdm_pi_tune = Prefs.d.parse(ctPi.getText()).floatValue(); }
//			catch (Exception e)	{
//				return error("Pi-tune: positive number expected.", ctPi);
//			}
			
//			try { Prefs.pdm_kappa_tune = Prefs.d5.parse(ctKap.getText()).floatValue(); }
//			catch (Exception e)	{
//				return error("Kappa-tune: positive number expected.", ctKap);
//			}
			
//			try { Prefs.pdm_ttp_tune = Prefs.d.parse(ctTTP.getText()).floatValue(); }
//			catch (Exception e)	{
//				return error("Ttp-tune: positive number expected.", ctTTP);
//			}
			
//			try { Prefs.pdm_gamma_tune = Prefs.d.parse(ctGam.getText()).floatValue(); }
//			catch (Exception e)	{
//				return error("Gamma-tune: positive number expected.", ctGam);
//			}
			
			try { Prefs.pdm_beta_tune = Prefs.d5.parse(ctBet.getText()).floatValue(); }
			catch (Exception e)	{
				return error("Beta-tune: positive number expected.", ctBet);
			}
			
			try { Prefs.pdm_outgroup = Integer.parseInt(cOutgroup.getText()); }
			catch (Exception e)	{
...
//			try { Prefs.pdm_max_initial_tree_height = Prefs.d.parse(cHeight.getText()).floatValue; }
//			catch (Exception e)	{
//				return error("Max-initial-tree-height: positive number expected.", cHeight);
//			}
