...
//	}
	...
	static void cluster(TreePanel panel)
	{
		...
		/*
		 * NameColouriser nc = new NameColouriser(); for (int i = 0; i <
								...
								+ "sequence list?", 0) == JOptionPane.YES_OPTION)
		{
			...
			// for (int i = 0; i < clusters.size(); i++)
			{
				...
				if (indices[i] == -1)
				{
					MsgBox
							.msg(
									"Unable to find one or more sequences (in this tree"
	...
	/*
	 * private static Vector getGroup(Vector groups, String name, boolean
	 * create) { for (int i = 0; i < groups.size(); i++) { // Get the group at
	 * this position Vector group = (Vector) groups.elementAt(i);
	 *  // Search for this sequence in this group for (int j = 0; j <
	 * group.size(); j++) { if (((String)group.elementAt(j)).equals(name))
	 * return group; } }
	...
	private static SequenceCluster getCluster(
			LinkedList<SequenceCluster> clusters, String name, boolean create)
	{
		for (SequenceCluster cluster : clusters)
