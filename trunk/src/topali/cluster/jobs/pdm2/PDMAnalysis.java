...
class PDMAnalysis extends AnalysisThread
{
	...
	// TODO: This needs reformatted into a throws Exception format for
	// AnalysisThread
	...
	private float doCalculations() throws Exception
	{
	...
	{
		// System.out.println("Looking for " + p1 + " in " + set1.length + " and
		// " + p2 + " in " + set2.length);
	...
	private void saveWindowResults(int num) throws Exception
	{
		...
		BufferedWriter outL = new BufferedWriter(new FileWriter(new File(
				wrkDir, "win" + num)));
		// File written to the head node's hard disk
		BufferedWriter outH = new BufferedWriter(new FileWriter(new File(
				treesDir, "win" + num)));
	...
	private void addNexusCommands() throws Exception
	{
		...
		out.newLine();
		out
				.write("  mcmcp ngen=10000 printfreq=100 samplefreq=50 nchain=4 savebrlens=yes;");
		out.newLine();
		out.write("  mcmc;");
		out.newLine();
	...
	private void writeScores(float[] scores) throws IOException
	{
