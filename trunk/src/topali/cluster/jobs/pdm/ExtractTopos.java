...
public class ExtractTopos
{
	...
	private void getArrayFromVector()
	{
		...
		System.out.println("topoArray has " + nRows + " rows and " + nCols
				+ " columns.");
	}

	// -------------------------------------------------------------
	// Read topologies from input file and store them in vector topoStrings
	private boolean readTopos(File filename) throws Exception
	{
	...
	public boolean addTopo(boolean writeToFile) throws Exception
	{
	...
	public String[][] showTopos()
	{
		...
		/*
		 * System.out.println(); for (int i = 0; i < topoArray.length; i++) {
		 * for (int j = 0; j < topoArray[i].length; j++)
		 * System.out.print(topoArray[i][j] + " "); System.out.println(""); }
		 * System.out.println(topoArray); System.out.println();
		 */
		return topoArray;
	}

	// -------------------------------------------------------------
	void addWindowedTopos(int windowSize, int stepSize) throws Exception
	{
		...
		// Read in data
		this.readTopos(new File(wrkDir, "run1.top")); // That gets -->
														// topoStrings
		...
		if (iEnd <= windowSize)
		{
			throw new JambeException(
					"ET01: Wrong parameters for addWindowedTopos");
		}

		for (iStart = 0; iStart <= iEnd - windowSize; iStart = iStart
				+ stepSize)
		{
	...
	// -------------------------------------------------------------
	/*
	 * private void printTopos(String fileName){ // --> Temporary version: Needs
	 * to be improved !! // Print out the topology array for the current
	 * iteration to // a file with the specified name. If this file already
	 * exist, // the program terminates and an error message is returned //
	 * (this makes sure that results are written out to a new file). // Since I
	 * have not yet found a way to write to the end // of a file, this method
	 * calls UNIX shell commands // to concatenate files.
	 * 
	 *  // if (fileName != "stercus"){ // System.err.println("At current, the
	 * file name must be stercus"); // System.exit(0); // }
	 * 
	 * File oFile; //FileWriter oFileWriter=null; (not deprecated, but doesn't
	 * work) FileOutputStream oFileWriter=null; //PrintWriter oPrintWriter=null;
	 * (not deprecated, but doesn't work) PrintStream oPrintWriter=null; oFile=
	 * new File("resultsAllTopos.out"); // if (!oFile.exists()){ ///
	 * System.err.println("ERROR_03: Create resultsAllTopos.out"); //
	 * System.exit(0); // }
	 *  // Write results out to file System.out.println("Open file "+fileName);
	 * try{ //oFileWriter= new FileWriter(fileName); (not deprecated, but
	 * doesn't work) oFileWriter= new FileOutputStream(fileName);
	 * //oPrintWriter= new PrintWriter(oFileWriter); (not deprecated, but
	 * doesn't work) oPrintWriter= new PrintStream(oFileWriter); } catch
	 * (IOException oException){ System.err.println("ERROR_04: Could not open
	 * output file"); System.exit(0); } for (int i=0; i<topoStrings.size();
	 * i++){ oPrintWriter.print(topoStrings.elementAt(i)+" "); }
	 * oPrintWriter.println();
	 * 
	 * oPrintWriter.close(); try{ oFileWriter.close(); } catch (IOException ex) {
	 * System.out.println("ERROR_05 in readTopos: Couldn't close file");
	 * System.exit(0); }
	 *  // --------- Use UNIX to write to end of file -----
	 * 
	 * try { BufferedReader in = new BufferedReader(new
	 * FileReader(Prefs.mcmc_scratch + Prefs.sepF+"stercus")); BufferedWriter
	 * out = new BufferedWriter(new FileWriter("resultsAllTopos.out", true));
	 *  // out.newLine(); int read = in.read(); while (read != -1) {
	 * out.write(read); read = in.read(); }
	 * 
	 * 
	 * out.close(); in.close();
	 *  // System.out.println("Delete stercus: " + new File(Prefs.mcmc_scratch +
	 * Prefs.sepF+"stercus").delete()); } catch (Exception e) {
	 * System.out.println("cat stercus: "+e); } /* Runtime myRuntime =
	 * Runtime.getRuntime(); Process proc=null; try { try{ proc=
	 * myRuntime.exec(new String[] {"/bin/sh","-c","cat resultsAllTopos.out
	 * stercus > stercus2"}); proc.waitFor(); proc= myRuntime.exec(new String[]
	 * {"/bin/sh","-c","mv stercus2 resultsAllTopos.out"}); proc.waitFor();
	 * proc= myRuntime.exec(new String[] {"/bin/sh","-c","rm stercus"});
	 * proc.waitFor(); }catch (InterruptedException interuptex){
	 * System.out.println("IOException: " + interuptex.toString());
	 * System.exit(0); } } catch (IOException e) {
	 * System.out.println("IOException: " + e.toString()); System.exit(0); }
	 */
	// }
	// ----------------------- END ---------------------------------
}
