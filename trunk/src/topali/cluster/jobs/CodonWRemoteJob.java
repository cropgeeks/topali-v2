...
public class CodonWRemoteJob extends RemoteJob
{
	...
	public CodonWRemoteJob(CodonWResult result, AlignmentData data)
	{
		super("topali-cw", result);
	...
	public String ws_submitJob() throws Exception
	{
		...
		call = getCall();
		call.setOperationName(new QName("topali-cw", "submit"));
		...
		Tracker.log("SUBMITTED", "CODONW", result.jobId, url);
	...
	{
		call = getCall();
		call.setOperationName(new QName("topali-cw", "getPercentageComplete"));
	...
	{
		call = getCall();
		call.setOperationName(new QName("topali-cw", "getResult"));

		String resultXML = (String) call.invoke(new Object[]
		{ result.jobId });
		result = (CodonWResult) Castor.unmarshall(resultXML);
