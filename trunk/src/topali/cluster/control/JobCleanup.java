...
//		Logger.getLogger("topali.cluster.info-log").setLevel(Level.OFF);
		...
		{
			System.out
					.println("Usage: topali.cluster.control.JobCleanup <job_dir> <fail_age> <ok_age>");
			System.out
					.println("  <job_dir>  = path to directory containing cluster jobs");
			System.out
					.println("  <fail_age> = age in days that a failed job can remain");
			System.out
					.println("  <ok_age>   = age in days that a completed job can remain");
			System.exit(0);
		}
